...
def evaluate(data_loader, model, device, task, epoch, mode, num_class):
    ...
    misclassified_paths = path_list[np.where(true_label_decode_list != prediction_decode_list)]
    ...
    if mode=='test':
        ...
        with open("misclassified_images_test.txt", "a") as file:
            file.write("-" * 10 + f" Epoch {epoch} " + "-" * 10 + "\n")
            file.write("\n".join(misclassified_paths.tolist()))
        os.system(f"rm -rf misclassified_images_test/*")
        csv_data = np.vstack((path_list, prediction_decode_list, true_label_decode_list)).T
        with open("test_details.csv",mode='w',newline='',encoding='utf8') as cfa:
            wf = csv.writer(cfa)
            wf.writerow(["image_path", "predicted_class", "actual_class"])
            for r in range(csv_data.shape[0]):
                wf.writerow(csv_data[r, :].tolist())
        os.makedirs("misclassified_images_test", exist_ok=True)
        for filename in path_list:
            os.system(f"cp {filename} misclassified_images_test/")
    else:
        os.system(f"rm -rf misclassified_images_val/*")
        csv_data = np.vstack((path_list, prediction_decode_list, true_label_decode_list)).T
        with open("val_details.csv",mode='w',newline='',encoding='utf8') as cfa:
            wf = csv.writer(cfa)
            wf.writerow(["image_path", "predicted_class", "actual_class"])
            for r in range(csv_data.shape[0]):
                wf.writerow(csv_data[r, :].tolist())
        os.makedirs("misclassified_images_val", exist_ok=True)
        for filename in path_list:
            os.system(f"cp {filename} misclassified_images_val/")
        # with open("misclassified_images_val.txt", "a") as file:
        #     file.write("-" * 10 + f" Epoch {epoch} " + "-" * 10 + "\n")
        #     file.write("\n".join(misclassified_paths.tolist()))
